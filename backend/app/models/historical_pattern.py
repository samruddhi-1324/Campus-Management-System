...
class HistoricalTrendRecord(Base, TimestampMixin):
    ...
    institution_id: Optional[Mapped[str]] = mapped_column(String(36), ForeignKey("institutions.id"), index=True, nullable=True)
    category_id: Optional[Mapped[str]] = mapped_column(String(36), ForeignKey("categories.id"), index=True, nullable=True)
    year: Mapped[int] = mapped_column(Integer, index=True, nullable=False)
    quarter: Optional[Mapped[int]] = mapped_column(Integer, nullable=True)
    month: Optional[Mapped[int]] = mapped_column(Integer, nullable=True)
    ...
    pattern_summary: Optional[Mapped[str]] = mapped_column(Text, nullable=True)
